...
const rules = reactive<FormRules<FormData>>({
  author: [{ required: true, message: "請輸入作者名", trigger: "blur" }],
  title: [{ required: true, message: "請輸入文章標題", trigger: "blur" }],
  summary: [{ required: true, message: "請輸入大綱", trigger: "blur" }],
...
const addArticle = async () => {
  await http.post("/article", formData.value);
  ElMessage.success("新增成功");
};
const editArticle = async () => {
  await http.put(`/article/${formData.value.id}`, formData.value);
...
const getArticles = async () => {
  try {
    const { data } = await http.get("/articles?type=knowledge");
...
const editHandler = async (rowData: FormData) => {
  ...
  // formData.value.image = 'https://i.imgur.com/6lqITdx_d.webp?maxwidth=760&fidelity=grand'
};
...
const onUpload = async (file: UploadFile) => {
  ...
  fileFormData.append("image", file.raw as File);
  const { data } = await http.post("/uploadImage", fileFormData);
...
<template>
    ...
    >
      ...
      <el-form-item label="標籤" prop="categoryId">
        <el-radio-group v-model="formData.categoryId">
          <el-radio :key="i._id" :label="i._id" v-for="i in categoryList">{{
            i.title
          }}</el-radio>
        </el-radio-group>
      </el-form-item>
  ...
  <div class="relative border-box">
    <span
      class="absolute z-10 w-full text-center h-40 flex items-center justify-center text-white drop-shadow tracking-wider text-5xl"
      >踏取後台文章系統</span
    >
    <el-image
      class="block w-full mb-4 h-40 rounded-md brightness-[0.5]"
      :src="bannerImage"
      :fit="'cover'"
    />
  </div>
  ...
  <el-button
    ...
    class="mb-5"
    type="primary"
    @click="showAddArticleDialog('add')"
    >新增文章</el-button
  >
  ...
  <el-table :data="articleList">
    ...
    <el-table-column prop="type" label="類型" width="100">
      <template #default="scope">
        <el-tag :type="scope.row.type === 'news' ? 'info' : 'success'">{{
          scope.row.type === "news" ? "新聞" : "知識文章"
        }}</el-tag>
      </template>
    </el-table-column>
